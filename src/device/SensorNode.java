...
public abstract class SensorNode extends DeviceWithRadio {
	...
	/**
	 * Constructor 1 Instanciate the sensor unit Instanciate the battery
	 */
	public SensorNode() {
		super();
		...
		if (WisenSimulation.weather) {
			...
			String batteryModelFile = this.getBatteryModelFileName();
			Battery.loadBatteryModelFromFile(batteryModelFile, bm);
			battery.setBatteryModel(bm);
		}
		// ------------------------------------------------------------------------------------------KADJOUH
	...
	public SensorNode(double x, double y, double z, double radius, double radioRadius, double suRadius, int id) {
		super(x, y, z, radius, radioRadius, id);
		// sensorUnit = new SensorUnit(this.longitude, this.latitude, suRadius,
		// this);
		battery = new Battery();
		withRadio = true;
		withSensor = true;
	...
	public SensorNode(String x, String y, String z, String radius, String radioRadius, String suRadius, int id) {
		...
		// sensorUnit = new SensorUnit(this.longitude, this.latitude,
		// Double.valueOf(suRadius), this);
		battery = new Battery();
		withRadio = true;
		withSensor = true;
		calculateRadioSpace();
		...
		if (visible) {
			...
			if (!isDead()) {
				...
				if (hide == 0 || hide == 5) {
					...
					int z = MapLayer.getMapViewer().getZoom();
					// RadialGradientPaint p = new RadialGradientPaint(center,
					// radius*(z<3&&z>0?(3/(z+1)):1), dist, colors);
					RadialGradientPaint p = new RadialGradientPaint(center,
							radius * (z < 3 && z > 0 ? (3 / (z + 1)) : 1), dist, colors);
					g2.setPaint(p);
					if (nPoint > 0)
	...
	public void draw(Graphics g) {
		if (visible) {
			...
			if (drawBatteryLevel) {
				...
				if (battery.getLevel() / battery.getInitialLevel() < 0.5)
					g.setColor(UColor.ORANGE);
				if (battery.getLevel() / battery.getInitialLevel() < 0.2)
					g.setColor(UColor.RED);
				// System.out.println();
				g.fillRect(x - 30, y - (int) (battery.getLevel() / battery.getInitialLevel() * 100. / 2.) + 25, 6,
						(int) (battery.getLevel() / battery.getInitialLevel() * 100. / 2.));
				g.setColor(Color.DARK_GRAY);
				...
				g.setColor(UColor.RED);
				g.fillRect(x - 20, y - (int) (bufferIndex * 1.0 / bufferSize * 100. / 2.) + 25, 6,
						(int) (bufferIndex * 1.0 / bufferSize * 100. / 2.));
				g.setColor(Color.DARK_GRAY);
	...
	public List<SensorNode> getSensorNodeNeighbors() {
		...
		for (SensorNode snode : DeviceList.getSensorNodes()) {
			if (((radioDetect(snode))) && this != snode && !this.isDead() && !snode.isDead() && sameCh(snode)
					&& sameNId(snode)) {
	...
	/*
	 * public void drawSendingReceiving(Graphics g, int x, int y) { if(drawTxRx)
	 * { for(int i=0; i<nPoint; i++) { g.drawLine(x, y, polyX[0][i],
	 * polyY[0][i]); } if (sending || receiving) { double alpha = 0.0; double
	 * alpha2 = Math.toDegrees(3.14); int as = 10; int sz = 30; for(int i=0;
	 * i<nPoint; i++) { if (sending) { g.setColor(Color.BLUE); as = 10;
	 * g.fillArc(polyX[0][i] - as, polyY[0][i] - as, as*2, as*2,180 - (int)
	 * alpha - as, as*2); as = 1; g.fillArc(polyX[0][i] - as*sz, polyY[0][i] -
	 * as*sz, as*sz*2, as*sz*2,180 - (int) alpha - as, as*2); alpha +=
	 * Math.toDegrees(deg); }
	 * 
	 * if(sending) { g.setColor(Color.RED); as = 10; g.fillArc(polyX[0][i] - as,
	 * polyY[0][i] - as, as*2, as*2,180 + (int) alpha2 - as, as*2); as = 1;
	 * g.fillArc(polyX[0][i] - as*sz, polyY[0][i] - as*sz, as*sz*2, as*sz*2,180
	 * + (int) alpha2 - as, as*2); alpha2 -= Math.toDegrees(deg); } } } } }
	 */
	...
	public void initForSimulation() {
		...
		setRadioLinkColor(UColor.colorTab2[0]);
