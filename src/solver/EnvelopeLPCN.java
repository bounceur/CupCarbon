					...
					if (!nodes.get(j).isDead()) {
						if ((current != j) && (n1.radioDetect(n2) || n2.radioDetect(n1))) {
								...
								while(k>0 && !intersection) {
									//complexity++;
									xp1 = nodes.get(DeviceList.getLastEnvelope().get(k-1)).getLatitude();
									yp1 = nodes.get(DeviceList.getLastEnvelope().get(k-1)).getLongitude();
									xp2 = nodes.get(DeviceList.getLastEnvelope().get(k)).getLatitude();
									yp2 = nodes.get(DeviceList.getLastEnvelope().get(k)).getLongitude();
